...
For local or proxied OpenAI-compatible endpoints, you can override the API base URL:

```bash
export TRADINGAGENTS_BACKEND_URL=http://localhost:1234/v1   # LM Studio example
```

For enterprise providers (e.g. Azure OpenAI, AWS Bedrock), copy `.env.enterprise.example` to `.env.enterprise` and fill in your credentials.

For local models, configure Ollama with `llm_provider: "ollama"` or LM Studio with
`llm_provider: "lmstudio"` in your config. For providers such as Qwen, GLM, Ollama,
and LM Studio, the CLI can also prompt for a custom OpenAI-compatible endpoint.
...
LM Studio example (Qwen 3.5 local):

```python
config = DEFAULT_CONFIG.copy()
config["llm_provider"] = "lmstudio"
config["backend_url"] = "http://localhost:1234/v1"
config["quick_think_llm"] = "qwen3.5"
config["deep_think_llm"] = "qwen3.5"

# Use the exact model ID exposed by your LM Studio server.
# If your model name is different (for example, qwen3.5:latest),
# set quick_think_llm and deep_think_llm to that exact ID.
```
