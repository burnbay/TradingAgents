...
class LocalLLMChatOpenAI(NormalizedChatOpenAI):
    """ChatOpenAI variant for local servers (Ollama, LM Studio).

    Local servers typically implement only a subset of the OpenAI API.
    In particular they reject ``tool_choice`` as an object (only the
    string values ``"none"``, ``"auto"``, ``"required"`` are accepted),
    which breaks LangChain's ``function_calling`` structured-output path.
    Raising ``NotImplementedError`` here lets ``bind_structured`` detect
    the incompatibility at agent-creation time and route every call
    through free-text generation instead of failing at invocation time.
    """

    def with_structured_output(self, schema, *, method=None, **kwargs):
        raise NotImplementedError(
            "Local LLM servers (Ollama / LM Studio) do not support the "
            "object form of tool_choice required for structured output. "
            "Free-text generation will be used instead."
        )
